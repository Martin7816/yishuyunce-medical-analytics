"""Compare the published cohorts rows with a snapshot artifact."""
...
def verify(snapshot_path: Path) -> dict[str, Any]:
    ...
    missing = sorted(set(expected_cohorts) - set(actual_cohorts))
    extra = sorted(set(actual_cohorts) - set(expected_cohorts))
    mismatches = []
    for key in sorted(set(expected_cohorts) & set(actual_cohorts)):
        actual = actual_cohorts[key]
        actual_payload = json.loads(actual["payload_json"])
        if (
            actual_payload != expected_cohorts[key]["payload"]
    ...
    result = {
        ...
        "expected_cohort_rows": len(expected_cohorts),
        ...
        "empty_combination_count": sum(
            ...
            for record in expected_cohorts.values()
        ),
    }
