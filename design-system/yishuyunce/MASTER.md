...
保留当前八个固定路由和深链接，不增加新的业务模块。导航分成三个有标题的组，但 URL 与顺序保持不变：

1. **总览与分析模块**：`/overview` 运营驾驶舱、`/hospitals` 医院运营分析、`/diseases` 疾病画像分析、`/cohorts` 住院记录群体分析、`/costs` 费用与成本分析、`/risks` 病情严重程度与风险分析、`/payments` 支付方式分析；
2. **辅助能力**：`/assistant` AI 问答与洞察报告。数据质量校验和高费用模型只保留后端接口，不提供独立前端页面。
