...
前端为住院运营分析提供十个页面入口。`OverviewPage` 提供两种有明确分工的阅读方式：浅色工作台用于完整指标、图表和专题下钻；`mode=screen` 用于笔记本窗口里的运营全景展示，只保留六个核心指标、批次摘要、结构扫描、集中度、费用关系和风险结构。两种模式读取同一已发布快照，但不复用同一版面叙事。七个专题页复用 `AnalysisPage`、`MetricCard`、`AnalyticsChart` 和 `PageState`，高费用记录分类与 AI 问答保留各自需要的表单和结果区。
...
|---|---|
| `/overview` | 浅色运营分析工作台 |
| `/overview?mode=screen` | 深色笔记本自适应运营全景展示（执行摘要与结构扫描） |
| `/hospitals` | 医院运营分析 |
| `/diseases` | 疾病画像分析 |
| `/cohorts` | 住院记录群体分析 |
| `/costs` | 费用与成本分析 |
...
- 展示模式采用普通网页流式布局，不锁定 1920×1080；笔记本窗口按宽度自适应，内容不足以一屏容纳时自然纵向滚动。它是面向汇报和快速观察的摘要视图，不替代 `/overview` 的完整分析工作台。
- 每个 D3 SVG 图表提供工具提示、直接数值、键盘选择和可展开的数据表替代视图。
