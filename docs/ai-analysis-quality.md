...
- 如果模型总结超时、返回非法结构或未通过证据校验，后端可以直接从 Safe Evidence 的分组和值及服务器已生成的 `derived_facts` 生成保守摘要，并保留原有证据 ID、数据版本和限制说明。排名问题在用户未明确指定 Top N 时至少读取前三项，可使用服务器已核验的第一名与第二名差值；模型本身仍不得临时计算新数字。
...
derived facts 只做可复核的加减、除法、排名、最大/最小、差距、总量和份额；除数为零时不生成比例。不计算显著性、相关系数、预测概率，也不作因果推断。Top-K 排名只生成返回项数量、返回项合计和第一名相对第二名的差值，不把 Top-K 合计冒充完整总体；只有证据明确标记为完整分布时才生成总体份额。
