...
本文定义疾病病例量 TOP10 的统计范围、清洗规则、排序方式和服务结果结构。数据任务、MySQL、API、页面和独立核对共同使用这些约定。统计范围或字段含义发生变化时，相关代码、测试和文档必须在同一任务中保持一致。
...
真实数据版本不使用个人绝对路径标识，而由来源文件名、大小和 SHA-256 指纹共同确定：
...
数据任务必须在运行前记录实际 `data_version`。文件名、大小或 SHA-256 任一项不同即表示不同的数据版本，服务结果和 TOP10 数值不得跨版本混用。固定脱敏样本另标记为 `fixture:sparcs_mvp_sample:v1`，只用于逻辑核对，不代表全量分布。
...
疾病病例量 TOP10 的 MySQL 服务表为 `disease_case_count_top10_result`，完整 DDL 和刷新模板位于 [`data/sql/001-mvp-disease-top10-service.sql`](../data/sql/001-mvp-disease-top10-service.sql)。表只保存当前已经校验并发布的一个完整批次，不创建历史批次表、原始明细表或重复指标表。这样 API 只需按 `rank ASC` 读取一张小结果表；`PRIMARY KEY(rank)`、`UNIQUE(data_version, rank)` 和 `UNIQUE(data_version, diagnosis_name)` 共同保证当前结果不会混入重复名次、重复名称或多个版本。

疾病病例量 TOP10 服务结果不包含患者 ID、患者数、原始住院明细、诊断代码、医院维度、费用/成本、住院时长、年龄性别分层、分类模型标签或 AI 字段；`diagnosis_code` 只在清洗过程用于追溯，不能被 API 或页面当作结果字段。其他分析维度由统一分析快照承载，不进入本表。
...
标准化的目标是消除不会改变名称含义的首尾空白，不替业务人员判断不同文本是否为同一疾病。输入文件读取时处理文件开头的 UTF-8 BOM；诊断值只按统一的首尾 Unicode 空白规则清理。非空的零宽字符或其他非空白控制字符不被静默删除、替换或语义解释，应计入质量检查并保持原文，其他处理方式必须先明确规则、影响和验收方法。
...
InnoDB 的事务可见性保证 API 不会读到删除后、插入前的半批数据；刷新失败时旧批次继续可读。发布成功后只保留一个完整服务批次；重复发布同一 `data_version` 会得到相同表内容。不同数据版本的记录不得混合写入。

该服务表不保存原始明细，也不提供批次审计功能。
...
- **数据任务与 MySQL**：使用 `disease_case_count_top10_result` 及其 DDL，按完整批次事务发布；保存 `data_version` 和 `generated_at`，失败时不写入半批数据。
- **Flask API**：见 [API 契约](05-api.md)。接口读取已生成的服务结果，沿用 `rank`、`diagnosis_name`、`case_count`、`unit`、`data_version` 和批次元数据；路由不复制清洗、聚合或排序。
- **Vue 页面**：病例量显示为住院出院记录数；名称、排名和数量按服务结果顺序展示。
- **端到端验收**：使用固定样例和全量版本分别核对数据任务、服务结果、API 和图表；任一层的排序、单位或数量不同都按公共契约问题处理。

本契约只描述疾病病例量 TOP10；医院、费用、住院时长、分类模型和 AI 使用统一分析快照及各自契约。
