...
Issue #7 已将下列规则冻结，完整字段、版本和边界说明见 [疾病病例量 TOP10 指标与数据契约](02-metrics-and-data-contract.md)。
...
7. `Length of Stay`、费用和其他字段先不进入第一轮分组逻辑，字段异常不改变本指标的计数规则。
...
- 对 #7：统计范围、空值排除、首尾空白清洗、重复记录、并列排序和严格 TOP10 截断已在 `docs/02-metrics-and-data-contract.md` 冻结；全量 TOP10 是该版本数据的可复查基线。
